 ...
 *
 * When `reSeedFromDataGrid` is true, an item that is already present in the
 * layout (because that breakpoint's layout was generated by cloning another
 * breakpoint's) is re-seeded from the child's `data-grid` size fields. This
 * stops an item from shrinking when the layout grows to a larger breakpoint
 * (#2110). Stored x/y positions are always preserved.
 */
  ...
  React.Children.forEach(children, child => {
    ...
    if (existingItem) {
      if (reSeedFromDataGrid && dataGrid) {
        // The layout was generated by cloning another breakpoint's layout.
        // Prefer the child's authored size (w/h and min/max) over the cloned
        // values so the item never shrinks on breakpoint growth, while keeping
        // the stored x/y position.
        layout.push({
          ...existingItem,
          i: key,
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        ...
        // Re-seed from children's data-grid only when the target breakpoint
        // layout did not exist and was generated by cloning another
        // breakpoint's layout. A stored layout is the user's own state and
        // must not be clobbered (#2110).
