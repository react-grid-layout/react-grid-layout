...
describe("wrapCompactor", () => {
  ...
  describe("onMove - moving item earlier in sequence", () => {
    it("shifts items down when element is moved earlier", () => {
      // Initial layout:
      // A B
      // C D
      // E F
      ...
      const layout: Layout = [
        ...
        { i: "C", x: 0, y: 1, w: 1, h: 1, static: false, moved: false },
        { i: "D", x: 1, y: 1, w: 1, h: 1, static: false, moved: false },
        { i: "E", x: 0, y: 2, w: 1, h: 1, static: false, moved: false },
        { i: "F", x: 1, y: 2, w: 1, h: 1, static: false, moved: false },
        { i: "G", x: 0, y: 3, w: 1, h: 1, static: false, moved: false },
        { i: "H", x: 1, y: 3, w: 1, h: 1, static: false, moved: false }
      ];

      // Move G (position 6) to position B (position 1)
      // Expected result:
      // A G
      // B C
      // D E
      ...
      const result = wrapCompactor.onMove(
        layout,
        itemG,
        1, // x - position of B
        0, // y - position of B
      ...
      expect(result.find(l => l.i === "A")).toMatchObject({ x: 0, y: 0 });
      expect(result.find(l => l.i === "G")).toMatchObject({ x: 1, y: 0 });
      expect(result.find(l => l.i === "B")).toMatchObject({ x: 0, y: 1 });
      expect(result.find(l => l.i === "C")).toMatchObject({ x: 1, y: 1 });
      expect(result.find(l => l.i === "D")).toMatchObject({ x: 0, y: 2 });
      expect(result.find(l => l.i === "E")).toMatchObject({ x: 1, y: 2 });
      expect(result.find(l => l.i === "F")).toMatchObject({ x: 0, y: 3 });
      expect(result.find(l => l.i === "H")).toMatchObject({ x: 1, y: 3 });
    });
  });

  describe("onMove - moving item later in sequence", () => {
    it("shifts items up when element is moved later", () => {
      // Initial layout:
      // A B
      // C D
      // E F
      ...
      // Move B (position 1) to position F (position 5)
      // Expected result:
      // A C
      // D E
      // F B
      ...
      expect(result.find(l => l.i === "A")).toMatchObject({ x: 0, y: 0 });
      expect(result.find(l => l.i === "C")).toMatchObject({ x: 1, y: 0 });
      expect(result.find(l => l.i === "D")).toMatchObject({ x: 0, y: 1 });
      expect(result.find(l => l.i === "E")).toMatchObject({ x: 1, y: 1 });
      expect(result.find(l => l.i === "F")).toMatchObject({ x: 0, y: 2 });
      expect(result.find(l => l.i === "B")).toMatchObject({ x: 1, y: 2 });
      expect(result.find(l => l.i === "G")).toMatchObject({ x: 0, y: 3 });
      expect(result.find(l => l.i === "H")).toMatchObject({ x: 1, y: 3 });
    });
  });

  describe("onMove - no position change", () => {
    it("returns layout with item at same position", () => {
      const layout: Layout = [
        { i: "A", x: 0, y: 0, w: 1, h: 1, static: false, moved: false },
        { i: "B", x: 1, y: 0, w: 1, h: 1, static: false, moved: false }
      ];
      ...
      const result = wrapCompactor.onMove(layout, itemA, 0, 0, 2);

      // Item should be at same position with moved flag set
  ...
  describe("onMove - non-existent item", () => {
    it("handles non-existent item gracefully", () => {
      ...
      const result = wrapCompactor.onMove(layout, nonExistentItem, 1, 1, 2);

      // Should return cloned layout unchanged
      expect(result).toHaveLength(1);
      expect(result[0]?.i).toBe("A");
    });
  });

  describe("onMove immutability", () => {
    it("returns new layout (immutable)", () => {
      ...
      const itemA = layout[0]!;
      const result = wrapCompactor.onMove(layout, itemA, 1, 0, 2);
...
describe("Compactor interface compliance", () => {
  it("has required properties", () => {
    ...
    expect(typeof wrapCompactor.onMove).toBe("function");
  });
