...
export default class ReactGridLayout extends React.Component<Props, State> {
  ...
  onDragStop: (i: string, x: number, y: number, GridDragEvent) => void = (
    ...
    const { oldDragItem } = this.state;
    let { layout } = this.state;
    const { cols, preventCollision, allowOverlap, fillGaps, lastRowGap, heightUnits } = this.props;
    const l = getLayoutItem(layout, i);
    if (!l) return;
  ...
  onResizeStop: (i: string, w: number, h: number, GridResizeEvent) => void = (
    ...
    const { layout, oldResizeItem } = this.state;
    const { cols, allowOverlap, fillGaps, lastRowGap, heightUnits } = this.props;
    const l = getLayoutItem(layout, i);
  ...
  render(): React.Element<"div"> {
    const { className, style, isDroppable, innerRef, fillGaps } = this.props;
