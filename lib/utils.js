...
export function fillInGaps(layout, columnCount, lastRow, heightUnits = 4) {
    ...
    const maxSection = !(maxY / heightUnits % 0) ? Math.round((maxY / heightUnits) + 1) : Math.round(maxY / heightUnits);
    ...
    // * 3. Fill matrix gaps
    for (let section = 1; section <= maxSection; section++) { // w by h section. we will split it in "rows" of heightUnits [i.e. the default card height is 4, so we will fill in 4 h sections]
      const startingY = section === 1 ? 0 : ((heightUnits * (section - 1)) );
      const endingY = (heightUnits * section);
      for (let y = startingY; y < endingY; y++) {             // *  Row section
        for (let x = 0; x < columnCount; x++) {                // * Cell loop -- Go through all cells in the row
          if (matrix[y][x] === null) {                         // If the cell is empty
            gaps.push(scan(matrix, { i: `g-${gaps.length}`, x, y, w: 1, h: 1,}, x, y, columnCount, startingY, endingY))    // Recursively grow into the largest shape
          }
        }
      }
    }
    ...
    lastRow && gaps.push({ w: columnCount, x: 0, y: maxY, h: heightUnits, i: 'gap-last-row', lastRow: true, ...gapConfig });
