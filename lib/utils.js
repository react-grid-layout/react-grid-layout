...
export function cloneLayoutItem(layoutItem: LayoutItem): LayoutItem {
  return {
    ...
    isAdaptable: layoutItem.isAdaptable,
    fixLayout: layoutItem.fixLayout,
...
export function correctFixLayout(layout: Layout, bounds: { cols: number }, containerHeight: number): Layout {
  return layout.map(item => {
    const l = JSON.parse(JSON.stringify(item))
    if (!l.fixLayout?.isFix) {
      return l;
    }
    if (typeof l.fixLayout.right === 'number') {
      l.x = bounds.cols - l.w - l.fixLayout.right;
    }
    if (typeof l.fixLayout.bottom === 'number') {
      if (l.isAdaptable) {
        l.y = (document?.documentElement?.clientHeight || window.innerHeight) - l.h * 1.0 *  (window.innerWidth > 800 ? 375 : window.innerWidth) / 375 - l.fixLayout.bottom;
      } else {
        l.y = containerHeight - l.h - l.fixLayout.bottom;
      }
    }
    if (typeof l.fixLayout.left === 'number') {
      l.x = l.fixLayout.left;
    }
    if (typeof l.fixLayout.top === 'number') {
      l.y = l.fixLayout.top;
    } 
    return l;
  });
...
export function setTransform({ top, left, width, height, i }: Position, isAdaptable: boolean, fixLayout: FixLayout, halfPage: string): Object {
  ...
  if (!isAdaptable) {
    ...
    return {
      ...
      width: `${width}px`,
      height: `${height}px`,
      position: fixLayout?.isFix ? "fixed" : "absolute"
    };
  }
  let translate;
  if (fixLayout?.isFix) {
    const visibleHeight = halfPage ? 74 : 100
    translate = `translate(${px2rem(left)},calc(${visibleHeight}vh - ${px2rem((document?.documentElement?.clientHeight || window.innerHeight) - top - 1)}))`;
  } else {
    translate = `translate(${px2rem(left)},${px2rem(top *  (window.innerWidth > 800 ? 375 : window.innerWidth) / 375)})`;
  }
  return {
    transform: translate,
    WebkitTransform: translate,
    ...
    OTransform: translate,
    width: px2rem(width),
    height: px2rem(height *  (window.innerWidth > 800 ? 375 : window.innerWidth) / 375),
    position: fixLayout?.isFix ? "fixed" : "absolute"
  };
}
...
export function synchronizeLayoutWithChildren(
  ...
  //  correctedLayout = correctFixLayout(correctedLayout, { cols: cols }, 0);
  return allowOverlap
    ? correctedLayout
