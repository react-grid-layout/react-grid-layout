...
export function childrenEqual(a: ReactChildren, b: ReactChildren): boolean {
  ...
  for (let i = 0; i < aArr.length; i++) {
    if (aArr[i]?.key !== bArr[i]?.key) return false;
    if (!deepEqual(aArr[i]?.props?.["data-grid"], bArr[i]?.props?.["data-grid"])) return false;
  }
  return true;
}
...
export function setTransform({ top, left, width, height, i }: Position, isAdaptable: boolean, fixLayout: FixLayout, halfPage: string): Object {
  ...
  if (fixLayout?.isFix) {
    ...
    if (typeof fixLayout.bottom === 'number') {
      translate = `translate(${px2rem(left)},calc(${visibleHeight}vh - ${px2rem(height + fixLayout.bottom)}))`;
    } else {
