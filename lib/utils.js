...
export function compact(
  ...
  if (compactType === "vertical") {
    ...
    compactVerticalFast(out, cols, allowOverlap);
...
/**
 * Compact a layout vertically using an efficient "rising tide" algorithm.
 *
...
function compactVerticalFast(
  ...
  sorted.sort((a, b) => {
    ...
    // In case the top left corner is the same,
    // sort static items in front to reduce number of collision checks.
    if (a.static) {
      if (!b.static) {
        return -1;
      }
  ...
  const tide: number[] = Array(cols);

  for (let x = 0; x < cols; ++x) {
    tide[x] = 0;
  }

  const staticItems = sorted.filter(item => item.static);
  ...
  for (let i = 0; i < numItems; i++) {
    ...
    if (item.static) {
      ...
      // Since static items will be visited in the same order
      ...
      // Fix smallest gap/resolve largest overlap (if overlaps are not allowed).
      ...
      for (let j = staticOffset; !allowOverlap && j < numStatics; ++j) {
        ...
        if (
          //staticItem.y < item.y + item.h && // This is implied above.
          staticItem.y + staticItem.h > item.y &&
