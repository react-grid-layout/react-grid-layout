...
export function cloneLayoutItem(layoutItem: LayoutItem): LayoutItem {
  return {
    ...
    gap: layoutItem.gap, // I want to change this to isGap
...
// heightUnits is how tall the gap should be?
 ...
 * @param  {Number} heightUnits   something to do with gap filler height??
 * @throw  {Array}                The new layout
 */
export function fillInGaps(layout, columnCount, fillGaps, lastRowGap, heightUnits = 4) {
    ...
    // Get maxY
    let maxY = 0;
    const itemsInLayout = layout.filter((l) => !l.gap).map((item) => {
    ...
    lastRowGap && gaps.push({ w: columnCount, x: 0, y: matrix.length, h: heightUnits, i: 'gap-last-row', ...gapConfig });
