...
export const getBackgroundGridSvgString = ({
  containerWidth,
  cols,
  margin,
  rowHeight,
  ...
  if (!containerWidth || !cols || !margin) {
    return "";
  }

  const svgWidth = containerWidth - 2 * (padding ? padding[0] : margin[0]);

  return `<svg xmlns="http://www.w3.org/2000/svg"
          width="${svgWidth + margin[0]}"
          height="${rowHeight + margin[1]}"
          fill="none"
          viewBox="${0} ${0} ${svgWidth + margin[0]} ${rowHeight - 1}" 
      ...
      >
          ${[...Array(cols).keys()]
            .map(i => {
              return `<rect 
                  x="${
                    ((svgWidth + margin[0]) / cols) * i +
                    (padding ? padding[0] : margin[0])
                  }"
                  y="0"
                  width="${(svgWidth + margin[0]) / cols - margin[0]}"
                  height="${rowHeight - margin[1]}"
                  rx="4"
                  fill="${gridColor}"
              />`;
...
export const getBgPaddingSvgString = ({
  margin,
  padding,
  containerWidth,
  bgColor = "#fff"
  ...
  if (!containerWidth || (!margin && !padding)) {
    return "";
  }

  const height = padding ? padding[1] : margin[1];

  return `<svg xmlns="http://www.w3.org/2000/svg"
          width="${containerWidth}"
          height="${padding ? padding[1] : margin[1]}"
          fill="none"
