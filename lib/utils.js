// @flow
export type LayoutItem = {w: number, h: number, x: number, y: number, i: string,
                          placeholder?: boolean, moved?: boolean, static?: boolean};
...
export function setTransform(style: Object, coords: [number, number]): Object {
  ...
  const x = ('' + coords[0]).replace(/(\d)$/, '$1px');
  const y = ('' + coords[1]).replace(/(\d)$/, '$1px');
...
export function synchronizeLayoutWithChildren(initialLayout: Layout, children: Array<React.Element>|React.Element,
  ...
  let layout: Layout = Array(children.length);
