...
export function setTransform({ top, left, width, height, i }: Position, isAdaptable: boolean, fixLayout: FixLayout): Object {
  ...
  let translate;
  if (fixLayout?.isFix) {
    translate = `translate(${px2rem(left)},calc(100vh - ${px2rem((document?.documentElement?.clientHeight || window.innerHeight) - top)}))`;
  } else {
