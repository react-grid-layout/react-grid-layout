 ...
 */
export function filterOutGaps(layout, lastRow = false){
    if (lastRow) return layout.filter((item) => !item.gap || item.lastRow);
    return layout.filter((item) => !item.gap || !item.lastRow);
}
 ...
 */
export function fillInGaps(layout, columnCount, lastRow) {
    const gapConfig = {
        gap: true,
        isDraggable: false,
        isResizable: false,
    ...
    let y;
    // Get all rows
    const layoutWithoutGaps = layout.filter((l) => !l.gap);
    layoutWithoutGaps.filter((l) => !l.gap).forEach((l) => {
        for ({ y } = l; y < l.y + l.h; y++) {
            if (rows[y]) rows[y].push(l);
            else rows[y] = [l];
        }
        if (maxY < y) maxY = y;
    });
    ...
    for (y = 0; y < maxY; y++) {
        ...
        for (let x = 0; x < columnCount; x++) {
            if (rows[y] && rows[y].find((l) => l.x <= x && l.x + l.w > x)) {
                if (w > 0) { // if a card fills the area
                    gaps.push({ x: x - w, y: y - emptyRows, w, h: 1, i: `gap-${gapNumber}`, ...gapConfig });
                    gapNumber++;
                    w = 0;
                }
            } else {
                w++; // no card fills the area
            }
        }
        if (w > 0) {
            gaps.push({ x: columnCount - w, y: y - emptyRows, w, h: 1, i: `gap-${gapNumber}`, ...gapConfig });
            gapNumber++;
        }
    }
    // If lastRow is true, add lastRow
    lastRow && gaps.push({ w: columnCount, x: 0, y: maxY, h: 1, i: 'gap-last-row', lastRow: true, ...gapConfig });
    return [...layoutWithoutGaps, ...gaps];  // Merge the layouts
}
