...
export function compact(layout: Layout, compactType: CompactType, cols: number): Layout {
  ...
  // sorted clone
  const sortedClone = cloneLayout(sorted);
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static) {
      l = compactItem(compareWith, l, compactType, cols, sortedClone);
...
/**
 *  Before moving item down it will check if the movement will cause collisions and move those items down before (repeated recurisivly)
 */
function resolveCompactionCollisionByMovingItemDown(item: LayoutItem, moveToY: number, layout: Layout) {
  item.y += 1;
  ...
  for (let i = itemIndex+1; i < layout.length; i++) {
    ...
    if (collides(item, otherItem)) {
      resolveCompactionCollisionByMovingItemDown(otherItem, moveToY + item.h, layout);
    }
  }
 ...
 *  Before moving item down it will check if the movement will cause collisions and move those items down before (repeated recurisivly)
 */
function resolveCompactionCollisionByMovingItemToTheRight(item: LayoutItem, moveToX: number, layout: Layout) {
  item.x += 1;
  const itemIndex = layout.indexOf(item);
  ...
  for (let i = itemIndex+1; i < layout.length; i++) {
    ...
    if (otherItem.x > (item.x + item.w)) {
      break;
    }

    if (collides(item, otherItem)) {
      resolveCompactionCollisionByMovingItemToTheRight(otherItem, moveToX + item.w, layout);
    }
  }
 ...
 */
export function compactItem(compareWith: Layout, l: LayoutItem, compactType: CompactType, cols: number, layout: Layout): LayoutItem {
  ...
  while((collides = getFirstCollision(compareWith, l))) {
    if (compactH) {
      resolveCompactionCollisionByMovingItemToTheRight(l, collides.x + collides.w, layout);
    } else {
      resolveCompactionCollisionByMovingItemDown(l, collides.y + collides.h, layout);
    }
