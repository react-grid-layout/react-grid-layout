...
var utils = module.exports = {
   ...
   * @param  {Boolean} horizontalCompact Whether or not to compact the layout horizontally.
   * @return {Array}       Compacted Layout.
   */
  compact(layout, verticalCompact, horizontalCompact) {
    ...
    for (var i = 0, len = sorted.length; i < len; i++) {
      ...
      if (!l.static) {
        l = utils.compactItem(compareWith, l, verticalCompact, horizontalCompact);
  ...
  /**
   * @param  {Array}   compareWith       Items flow around the items in the compareWith array.
   * @param  {Object}  l                 Layout item.
   * @param  {Boolean} horizontalCompact Whether or not to compact the layout horizontally.
   * @param  {Boolean} verticalCompact   Whether or not to compact the layout vertically.
   * @return {Object}                    Compacted LayoutItem.
   */
  compactItem(compareWith, l, verticalCompact, horizontalCompact) {
    // Move the element up and then to the left as far as it can go without colliding.
    return utils.compactDirection(compareWith, utils.compactDirection(compareWith, l, 'y', verticalCompact), 'x', horizontalCompact);
  },

  /**
   * @param  {Array}   compareWith Items flow around the items in the compareWith array.
   * @param  {Object}  l           Layout item.
   * @param  {String}  dir         Use y for vertical and x for horizontal.
   * @param  {Boolean} compact     Whether or not to compact the layout.
   * @return {Object}              Compacted LayoutItem.
   */
  compactDirection(compareWith, l, dir, compact) {
    var dimension = dir === 'x' ? 'w' : 'h';
    var collides;

    if (compact) {
      // Move the element up as far as it can go without colliding.
      while (l[dir] > 0 && !utils.getFirstCollision(compareWith, l)) {
        l[dir]--;
      }
    }
    ...
    while((collides = utils.getFirstCollision(compareWith, l))) {
      l[dir] = collides[dir] + collides[dimension];
    }
   ...
   * @param  {Boolean} verticalCompact Whether or not to compact the layout vertically
   * @param  {Boolean} horizontalCompact Whether or not to compact the layout horizontally after compacting vertically
   *   vertically.
   * @return {Array}                Working layout.
   */
  synchronizeLayoutWithChildren(initialLayout, children, cols, verticalCompact, horizontalCompact) {
    ...
    layout = utils.compact(layout, verticalCompact, horizontalCompact);
    return layout;
  },
