...
function shouldFillCell (matrix, gap, x,y, nextX, nextY, prevX, prevY, endX, startY, endY) {
  ...
  if(y === startY) { // if we are at the top of the section
    ...
    // if(agrees(t, id) || agrees(r, id)) {
      return shouldFill = true;
    // }
  } else if(x === endX - 1) { // if we are at the right of the section
    // tl t |
    //  l _ |
    // bl b |
...
function scan (matrix, gap, x, y, endX, startY, endY, heightUnits) {
  if(x < endX && y <= endY && (y - gap.y) < heightUnits && matrix[y][x] === null) { // * if the current cell is a gap
      ...
      for (let y = 0; y < matrix.length; y++) {             // *  Row section
        for (let x = 0; x < matrix[y].length; x++) {                // * Cell loop -- Go through all cells in the row
          if (matrix[y][x] === null) {                         // If the cell is empty
            gaps.push(scan(matrix, { i: `gap-${gaps.length}`, x, y, w: 1, h: 1,}, x, y, columnCount, y, maxY - 1, heightUnits))  // Recursively grow into the largest shape
          }
        }
      }
    printMatrix(matrix)
