...
function generateMatrix(layout: Layout, { x, y }): Array<Array<string>> {
  const matrix = [];
  for(let i=0; i<y; i++) {
      matrix[i] = [];
      for(var j=0; j<x; j++) {
...
function calculateHeightScore(matrix: Array<Array<string>>, initialX: number, y: number, dx: number) {
  let hScore = 1;
  if (initialX === 0) {
