...
function generateGridMatrix(layout: Layout, { width, height }): Array<Array<string>> {
  const matrix = [];
...
function generateGapItem(
  ...
  const heightScores = [calculateHeightScore(matrix, initialX, initialY, w)];
  let h = 2;
  for (let y = initialY + h - 1; y < matrix.length; y++) {
  ...
  return { i, w, h, x: initialX, y: initialY, isGap: true, isDraggable: false, isResizable: false };
}
...
export function fillInGaps(
  ...
  const matrix = generateGridMatrix(layout, {width: columnCount, height: maxY});
