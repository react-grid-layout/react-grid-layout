...
export function correctFixLayout(layout: Layout, bounds: { cols: number }, containerHeight: number): Layout {
  return layout.map(item => {
    ...
    if (typeof l.fixLayout.bottom === 'number') {
      if (l.isAdaptable) {
        l.y = (document?.documentElement?.clientHeight || window.innerHeight) - l.h - l.fixLayout.bottom;
      } else {
...
export function setTransform({ top, left, width, height }: Position, isAdaptable: boolean, fixLayout: FixLayout): Object {
  ...
  const translate = `translate(${px2rem(left)},${px2rem(top *  (window.innerWidth > 800 ? 375 : window.innerWidth) / 375)})`;
  return {
    transform: translate,
    WebkitTransform: translate,
    MozTransform: translate,
