 ...
 */
export function moveElement(layout, l, x: ?number, y: ?number, isUserAction: ?boolean) {
    if (l.static) {
        return layout
    }
...
export function moveElementAwayFromCollision(layout, collidesWith,
                                             itemToMove, isUserAction: ?boolean) {
 ...
 */
export function synchronizeLayoutWithChildren(initialLayout, children, cols, verticalCompact, breakpoint) {
    ...
    React.Children.forEach(children, (child, i) => {
        ...
        const exists = getLayoutItem(initialLayout, child.key || "1")
        if (exists) {
            layout[i] = cloneLayoutItem(exists)
        } else {
            const grid = child.props['data-grid'][breakpoint]

            // Hey, this item has a data-grid property, use it.
            if (grid) {
                if (!isProduction) {
                    validateLayout([grid], 'ReactGridLayout.children')
                }
                layout[i] = cloneLayoutItem({...grid, i: child.key})
            } else {
 ...
 */
export function validateLayout(layout, contextName): void {
