...
// TODO: our functions need typescript types
// TODO: understand these functions better
// creates a 2d array of where the bottom corners of items are?
const generateMatrix =(layout, { x, y }) => {
  ...
  layout.filter((l) => !l.isGap).forEach((l) => {
    const { x, y, w, h, i } = l;
    const endX = x + w;
    const endY = y + h;
...
function calculateHeightScore(matrix, initialX, y, dx) {
  ...
  if (initialX === 0) {
      ...
      if (y === matrix.length - 1){
          if (!(matrix[y][initialX - 1] === null || matrix[y][initialX - 1].includes('isGap'))) hScore++;
      } else {
  ...
  if (initialX + dx === matrix[y].length) {
      ...
      if (y === matrix.length - 1){
          if (!(matrix[y][initialX + dx] === null || matrix[y][initialX + dx].includes('isGap'))) hScore++;
      } else {
...
// creates an item to fill a gap
// also mutates the matrix
function generateGap(matrix, initialY, initialX, i, gapFillHeight) {
    let h = 0;
    ...
    return { i, x: initialX, y: initialY, w, h, isGap: true };
}
 ...
 */
export function fillInGaps(layout, columnCount, fillGaps, lastRowGap, gapFillHeight) {
    ...
    // Get maxY
    let maxY = 0;
    const itemsInLayout = layout.filter((l) => !l.isGap).map((item) => {
      const itemMaxCoord = item.y + item.h;
      maxY = (itemMaxCoord > maxY) ? item.y + item.h : maxY;
      return item;
    });

    // Create a new matrix with a Y of maxY and an X of columnCount
    const matrix = generateMatrix(layout, {x: columnCount, y: maxY});

    const gaps = [];
    for (let y = 0; y < matrix.length; y++) {
      for (let x = 0; x < matrix[y].length; x++) {
        if (matrix[y][x] === null) {
          gaps.push(generateGap(matrix, y, x, `gap-${gaps.length}`, gapFillHeight))
        }
      }
    }

    lastRowGap && gaps.push({ w: columnCount, x: 0, y: matrix.length, h: gapFillHeight, i: 'gap-last-row', ...gapConfig });
    return [...itemsInLayout, ...gaps];  // Merge the layouts
}
