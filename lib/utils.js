...
function shouldFillCell (matrix, gap, x,y, nextX, nextY, prevX, prevY, endX, startY, endY) {
  const id = gap.i;
  const hasPrevX = prevX !== null;
  const hasPrevY = prevY !== null;
  const hasNextX = nextX !== null && nextX < endX;
  const hasNextY = nextY !== null && nextY < endY;
  if(hasNextX) gap.w = gap.w + 1;
  if(hasNextY) gap.h = gap.h + 1;
  // * 1. Get adjacent cells
  // tl t tr
  //  l _ r
  // bl b br
  let tl, t, tr, r, br, b, bl, l, shouldFill;
  ...
  if(y === startY) { // if we are at the top of the section
    if(!hasPrevX) {
      //  -----
      //  | _ r
      //  | b br
  ...
  } else if(y === endY) { // if we are at the bottom of the section
  ...
  } else if(x === 0) { // if we are at the left of the section
    // | t tr
    // | _ r
    // | b br
    // if(agrees(t, id) || agrees(r, id)) {
      return shouldFill = true;
    // }
  } else if(x === endX) { // if we are at the right of the section
    // tl t |
    //  l _ |
    // bl b |
    if(agrees(tl, id) && !agrees(t, id) && agrees(l, id) ){
    ...
    return shouldFill = true;
  } else {  // if we are in some middle area
    // tl t tr
    //  l _ r
    // bl b br
...
function scan (matrix, gap, x, y, endX, startY, endY) {
  if(x <= endX && y < endY && matrix[y][x] === null) { // * if the current cell is a gap
    const prevX = (x !== 0) ? x - 1 : null;
    const prevY = (y !== 0) ? y - 1 : null;
    const nextX = (x < endX) ? x + 1 : null;
    const nextY = (y < endY) ? y + 1 : null;
    if (shouldFillCell(matrix, gap, x, y, nextX, nextY, prevX, prevY, endX, startY, endY)) { //
      matrix[y][x] = gap.i;
      if(nextY !== null && y !== endY) { // if we can keep doing down
        scan(matrix, gap, x, nextY, endX, startY, endY); // scan down
      }
      if(nextX !==null) { // if we can keep going right
...
function generateGaps(matrix){
  const gapDict = {};
  for (let h = 0; h <  matrix.length; h++) {                  // *  Row section
    for (let w = 0; w < matrix[h].length; w++) {              // * Cell loop -- Go through all cells in the row
      if (matrix[h][w].includes('gap-')) {
          if(!gapDict[matrix[h][w]])  {
            gapDict[matrix[h][w]] = {
              i: matrix[h][w],
              x: w,
              y: h,
          ...
          const gap = gapDict[matrix[h][w]];
          gap.w = w - gap.x + 1;
          gap.h = h - gap.y + 1;
          gapDict[matrix[h][w]] = gap;
      }
    }
  }
...
export function fillInGaps(layout, columnCount, lastRow, heightUnits = 4) {
    ...
    const maxSection = !(maxY / heightUnits % 0) ? Math.round((maxY / heightUnits)) : Math.round(maxY / heightUnits) + 1;
    const matrix = generateMatrix(layout, {x: columnCount, y: maxSection * heightUnits}); // * 2. Create a new matrix with a Y of maxY and an X of columnCount
    // * 3. Fill matrix gaps
    for (let section = 0; section < maxSection; section++) { // w by h section. we will split it in "rows" of heightUnits [i.e. the default card height is 4, so we will fill in 4 h sections]
      const startingY = (heightUnits * section);
      const endingY = (heightUnits * (section + 1));
      for (let y = startingY; y < endingY; y++) {             // *  Row section
        for (let x = 0; x < columnCount; x++) {                // * Cell loop -- Go through all cells in the row
          if (matrix[y][x] === null) {                         // If the cell is empty
            gaps.push(scan(matrix, { i: `gap-${gaps.length}`, x, y, w: 1, h: 1,}, x, y, columnCount, startingY, endingY))  // Recursively grow into the largest shape
          }
        }
      }
    }
