...
export function moveElement(
  ...
  // nearest collision.
  let sorted = sortLayoutItems(layout, compactType).filter((item) => dragging && !item.hideOnDrag);
  const movingUp =
...
export function filterOutGaps(arr, lastRow = false){
    if (lastRow) return arr.filter((item) => !item.i.includes('gap') || item.lastRow);
    return arr.filter((item) => !item.i.includes('gap') || !item.lastRow);
}
export function getOnlyGaps(arr, lastRowOnly = false){
    if (lastRowOnly) return arr.filter((item) => item.lastRow);
    return arr.filter((item) => item.i.includes('gap') || item.lastRow);
}
export function fillGaps(arr, columnCount, noLastRow) {
    const rows = {};
    let maxY = 0;
    let y;
    arr.forEach((config) => {
        for ({ y } = config; y < config.y + config.h; y++) {
            // eslint-disable-line no-plusplus
            if (rows[y]) rows[y].push(config);
            else rows[y] = [config];
        }
        if (maxY < y) maxY = y;
    });
    ...
    for (y = 0; y < maxY; y++) {
        ...
        if (rows[y]) {
            ...
            for (let x = 0; x < columnCount; x++) {
                // eslint-disable-line no-plusplus
                if (rows[y].find((config) => config.x <= x && config.x + config.w > x)) {
                    ...
                    w++; // eslint-disable-line no-plusplus
                }
            }
            if (w > 0) {
                emptySpaces.push({
