...
export function calcGridColWidth(positionParams: PositionParams): number {
  ...
  const containerPaddingX =
    containerPadding.length === 2
      ...
      : containerPadding[1] + containerPadding[3];
  return (containerWidth - margin[0] * (cols - 1) - containerPaddingX) / cols;
}
...
export function calcGridItemPosition(
  ...
  // Otherwise, calculate from grid units.
    ...
    out.top = Math.round((rowHeight + margin[1]) * y + containerPaddingTop);
    out.left = Math.round((colWidth + margin[0]) * x + containerPaddingLeft);
  }
...
export function calcXY(
  ...
  // left = containerPaddingLeft + x * (colWidth + marginX)
  // x * (colWidth + marginX) = left - containerPaddingLeft
  // x = (left - containerPaddingLeft) / (colWidth + marginX)
  let x = Math.round((left - containerPaddingLeft) / (colWidth + margin[0]));
  let y = Math.round((top - containerPaddingTop) / (rowHeight + margin[1]));
